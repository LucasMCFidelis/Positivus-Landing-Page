...
const ContactUsInput = ({ title, ...rest }: ContactUsInputProps) => (
    <div className="flex flex-col gap-2">
        <label htmlFor={title.toLowerCase()}>{title + (rest.required === true ? "*" : "")}</label>
        <input
            {...rest}
...
export function SectionContactUs() {
    return (
        <>
            ...
            <Card.Wrapper bgColor="primary" border="none" rounded="lg" className="relative w-full h-[80vh] grid grid-cols-1 lg:grid-cols-2">
                <form action="" className="flex flex-col text-paragraph-mobile lg:text-paragraph-desktop gap-5 p-8">
                    <ContactUsInput title={"Name"} />
                    <ContactUsInput title={"Email"} required />
                    <ContactUsTextarea title={"Message"} required />

                    <Button colorBG={"dark"} text="Send Message" type="submit" />
                </form>
                <img src="/public/assets/contactUsImage.svg" alt="Contact Us" className="absolute hidden h-4/5 w-3/5 lg:block top-[10%] -right-[28%]" />
            </Card.Wrapper>
